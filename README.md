...
### 🗃️ Структура базы данных
 Users — таблица пользователей: ```bashUserId, Login, Password, Role.```<br>
 MaterialTypes — таблица типов материалов: ```MaterialTypeId, MaterialTypeName, DefectPercentage.```<br>
 ProductTypes — таблица типов продукции: ```ProductTypeId, ProductTypeName, ProductTypeCoefficient.```<br>
 Partners — таблица партнеров: ```PartnerId, CompanyType, CompanyName, DirectorName, Email, Phone, Address, INN, Rating.```<br>
 Products — таблица продукции: ```ProductId, ProductTypeId, ProductName, ArticleNumber, MinimumCost.```<br>
 Sales — таблица продаж: ```SaleId, PartnerId, ProductId, Quantity, SaleDate, SalePrice.```<br>
